...
const form = reactive({
    assign_to: props.task ? props.task.assign_to : '',
    dead_line: props.task ? props.task.dead_line : '',
    description: props.task ? props.task.description : '',
    status: props.task ? props.task.status : 'incomplete',
    assign_by: auth.value.user.id,
});

const submit = () => {
    const url = isEditMode.value ? route('tasks.update', { task: props.task.id }) : route('tasks.store');
